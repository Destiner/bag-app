...
import purpleGlowImage from "@/assets/images/purple-glow.png";
// @ts-ignore
import Image from "@/components/frame/nft-sale/Success.vue";
...
export default defineEventHandler(async (event) => {
  ...
  const purpleGlowImageBuffer = base64ImageToBuffer(purpleGlowImage);
  const images: Buffer[] = [];
  if (bgImageBuffer) {
    images.push(bgImageBuffer);
  }
  if (purpleGlowImageBuffer) {
    images.push(purpleGlowImageBuffer);
  }
  if (bagImageBuffer) {
